...
function extractName(url) {
    ...
    try {
        ...
        return remarks ? decodeURIComponent(remarks) : '';
    } catch {
        return '';
    }
}
...
function parseHostPort(hostPort) {
    hostPort = safeDecodeURIComponent(hostPort);
    // 处理 IPv6: [::1]:port
    ...
    if (lastColon > 0 && hostPort.indexOf(':') !== lastColon) {
        return {
            ...
            port: parseInt(hostPort.substring(lastColon + 1)) || 443
        };
    }
...
function parseVlessUrl(url) {
    try {
        ...
        const body = safeDecodeURIComponent(url.substring(8)); // 去掉 vless://
        const atIndex = body.indexOf('@');
        if (atIndex === -1) {
            const hashIndex = body.indexOf('#');
            const queryIndex = body.indexOf('?');
            const coreEnd = [hashIndex, queryIndex].filter(index => index !== -1).sort((a, b) => a - b)[0] ?? body.length;
            const encodedCore = body.substring(0, coreEnd);
            const suffix = body.substring(coreEnd);
            const decodedBody = base64Decode(encodedCore);
            return decodedBody && decodedBody.includes('@') ? parseVlessUrl(`vless://${decodedBody}${suffix}`) : null;
        }

        let uuid = body.substring(0, atIndex);
        if (uuid.includes(':')) uuid = uuid.split(':').pop();
        ...
        if (security === 'none' && params.get('pbk')) security = 'reality';
        ...
        if (params.get('tls') === '1' || params.get('tls') === 'true' || params.get('tls') === 'tls') {
            proxy.tls = true;
        }
...
// SNI (支持 sni 和 peer 两种参数名，Shadowrocket 使用 peer)
      if (params.get('sni')) {
        ...
        } else if (params.get('xtls') === '2') {
            proxy.flow = 'xtls-rprx-vision';
        }
        if (proxy.tls && network === 'ws' && !proxy.servername && params.get('host')) {
            proxy.servername = params.get('host');
            proxy.sni = params.get('host');
        }
