 ...
 * @returns {Object[]}
 */
function normalizeClashProxyGroups(proxyGroups) {
    return proxyGroups.map(group => {
        if (!group || typeof group !== 'object' || !group.options) {
            return group;
        }

        const { options: groupOptions, ...publicGroup } = group;
        return {
            ...publicGroup,
            ...groupOptions
        };
...
export function generateBuiltinClashConfig(nodeList, options = {}) {
    ...
    try {
        ...
        const config = {
            'mixed-port': 7890,
            'allow-lan': levelKey === 'MATRIX' ? false : true,
            ...(levelKey === 'MATRIX' ? {
                'bind-address': '*',
                'ipv6': false,
                'unified-delay': true,
            ...
            'mode': 'rule',
            'log-level': 'info',
            'external-controller': levelKey === 'MATRIX' ? '127.0.0.1:9090' : ':9090',

            'dns': {
                'enable': true,
                'listen': '0.0.0.0:1053',
                ...(levelKey === 'MATRIX' ? { 'ipv6': false } : {}),
                'default-nameserver': ['223.5.5.5', '1.1.1.1'],
                'enhanced-mode': 'fake-ip',
                'fake-ip-range': '198.18.0.1/16',
                'fake-ip-filter': levelKey === 'MATRIX' ? ['*.lan', '*.local', 'localhost.ptlogin2.qq.com'] : ['*.lan', '*.localhost'],
                'nameserver': [
            ...
            'profile': {
                'store-selected': true,
                ...(levelKey === 'MATRIX' ? { 'store-fake-ip': true } : {}),
